...
class Picking(models.Model):
    ...
    def _get_forced_pack_op_mapping_key(self, product, computed_putaway_locations):
        """Used to hook the pack operation grouping key
        """
    ...
    def _get_pack_op_mapping_key(self, quant, computed_putaway_locations):
        """Used to hook the pack operation grouping key
        """
        _Mapping = namedtuple('Mapping', ('product', 'package', 'owner', 'location', 'location_dst_id'))
    ...
    def _prepare_pack_ops(self, quants, forced_qties):
        ...
        for quant in valid_quants:
            key = self._get_pack_op_mapping_key(quant, computed_putaway_locations)
        ...
        for product, qty in forced_qties.items():
            if qty <= 0.0:
                continue
            key = self._get_forced_pack_op_mapping_key(product, computed_putaway_locations)
            qtys_grouped.setdefault(key, 0.0)
